...
def test(epoch):
    ...
    net.eval()
    print('Extracting Gallery Feature...')  # 测试集
    start = time.time()
    ptr = 0
    if args.pcb == 'on':
        feat_dim = args.num_strips * args.local_feat_dim  # 6*512=3072
    else:
        feat_dim = 2048
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(gall_loader):
            ...
            if args.pcb == 'on':
                # 在这里加上模型权重
                weights_dict = torch.load('save_model/sysu_conv4.t')
                b = weights_dict['net']
                net.load_state_dict(b)

                feat, visualize_x = net(input, input, test_mode[0])  # (二维数据 test_mode[0]=1)
                gall_feat[ptr:ptr + batch_num, :] = feat.detach().cpu().numpy()
                label_v = [str(i) for i in range(101)]
                plot(visualize_x, input, label_v)
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(query_loader):
            ...
            if args.pcb == 'on':
                ...
                feat, visualize_x = net(input, input, test_mode[1])
                query_feat[ptr:ptr + batch_num, :] = feat.detach().cpu().numpy()
                label_v = [str(i) for i in range(64)]
                plot(visualize_x, input, label_v)
            else:
...
# print('Rank-1: {:.2%} |  mAP: {:.2%}| mINP: {:.2%}'.format(best_acc, best_mAP, best_mINP))
