...
from utils import *
from loss import OriTripletLoss,  CenterTripletLoss, CrossEntropyLabelSmooth, TripletLoss_WRT, BarlowTwins_loss, TripletLoss, local_loss_idx, global_loss_idx
...
def train(epoch):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        if args.pcb ==  'on':
            ...
            # feat, out0, feat_all,  global_feature_extract, local_feat_extract= net(input1, input2)
            ...
            # total loss
            correct += batch_acc
            loss = loss_id + loss_tri + 0.4*loss_twins + loss_local_loss_inx + loss_tri_relation #loss_id_relation +#+ loss_global_loss_inx
            # loss = loss_id + loss_tri + loss_local_loss_inx + loss_global_loss_inx + loss_tri_relation  #IRGM+CFAM
            # loss = loss_id + loss_tri + loss_local_loss_inx + loss_global_loss_inx         # CFAM
            # loss = loss_id + loss_tri + loss_twins + loss_tri_relation # HC + SSL + relationship
            # loss = loss_id + loss_tri + loss_twins # HC + SSL        # CMCC
            # loss = loss_id + loss_tri + loss_tri_relation  # IRGM
            # loss = loss_id + loss_tri + loss_tri_relation + loss_local_loss_inx + loss_global_loss_inx     # IRGM+CFAM
...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(gall_loader):
            ...
            ptr = ptr + batch_num
        sio.savemat('gallery.mat', {'result': gall_feat, 'labels': gall_label})
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(query_loader):
            ...
            ptr = ptr + batch_num
        sio.savemat('query.mat', {'result2': query_feat, 'labels': query_label})
    print('Extracting Time:\t {:.3f}'.format(time.time() - start))
...
for epoch in range(start_epoch, 71 if args.dataset == 'regdb' else 71  - start_epoch):
